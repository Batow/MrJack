...
describe('Test Facade',function(){
	...
	describe('Test de deplacement', function(){
		...
		it('global',function(){
			...
			assert.equal(result.ok,"true","Une construction correcte ne fonctionne pas : " + result.message);
		})
	});
	...
	describe('Test de rotation', function(){
		...
		it('id_partie', function(){
			//On vérifie que l'identifiant de la partie n'est pas null
			assert.equal(instance.rotation(null,plateau.joueurCourant,1,2).ok,"false","id_partie ne doit pas être null");
			//On vérifie que l'identifiant de la partie existe
			assert.equal(instance.rotation("toto","batow",1,2).ok,"false","id_partie n'existe pas");	
		})

		//Tests sur le paramètre 'id_joueur'
		it('id_joueur',function(){
			//On vérifie que l'identifiant du joueur n'est pas null
			assert.equal(instance.rotation(plateau.id_plateau,null,1,2).ok,"false","id_joueur ne doit pas être null");
			//On vérifie que l'identifiant du joueur existe
			assert.equal(instance.rotation(plateau.id_plateau,"toto",1,2).ok,"false","id_joueur n'existe pas");	
		})

		//Tests sur le paramètre 'nb_rotation'
		it('nb_rotation', function(){
			//On vérifie que le paramètre n'est pas null 
			assert.equal(instance.rotation(plateau.id_plateau,plateau.joueurCourant,1,null).ok,"false","nb_rotation ne doit pas être null");
			//On vérifie que l'on saisie un nombre
			assert.equal(instance.rotation(plateau.id_plateau,plateau.joueurCourant,1,"tata").ok,"false", "nb_rotation doit être un nombre");
			//On vérifie que l'on saisie un entier
			assert.equal(instance.rotation(plateau.id_plateau,plateau.joueurCourant,1,1.3).ok,"false", "nb_rotation doit être un entier");
		})
